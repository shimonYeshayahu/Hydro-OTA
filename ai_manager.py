...
def process_all_controllers():
    ...
    for controller_id, data in all_controllers.items():
        ...
        prompt = f"""
            ...
            היסטוריית מדדים (דגימה כל חצי שעה):
            {history_text}

            הנחיות קריטיות לניתוח:
            1. אסור לך בשום אופן לטעון ש"אין מספיק נתונים" או להתעלם מהנתונים הקיימים! חובה עליך לנתח כל דגימה שסופקה לך, גם אם מדובר ביום אחד או יומיים בלבד.
            2. פתח את הדוח בציון מפורש של כמות הזמן שהנתונים מכסים (למשל: "על בסיס הנתונים מ-48 השעות האחרונות...").
            3. נתח את הנתונים הקיימים: האם המדדים יציבים ובטווח? 
            4. האם יש חריגות או מגמות בתוך טווח הזמן שניתן לך שמצריכות התערבות? 
            5. סכם במספר המלצות פרקטיות למגדל.
            
            החזר אך ורק את תוכן הדוח.
            """
